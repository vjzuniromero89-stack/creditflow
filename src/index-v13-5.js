// CreditFlow v15 — Smart Credit Repair + Client Intake
...
function randomToken(){
  const a=new Uint8Array(24);crypto.getRandomValues(a);return Array.from(a,x=>x.toString(16).padStart(2,"0")).join("");
}
async function saveIntakeImage(db,clientId,docType,file){
  if(!file||typeof file==="string"||!file.size)return;
  if(file.size>1.5*1024*1024)throw new Error("Cada foto debe pesar máximo 1.5 MB.");
  const allowed=new Set(["image/jpeg","image/png","image/webp"]);if(!allowed.has(file.type))throw new Error("Las fotos deben ser JPG, PNG o WEBP.");
  ...
  const data=btoa(bin);
  await db.prepare(`DELETE FROM client_documents WHERE client_id=? AND doc_type=?`).bind(clientId,docType).run();
  await db.prepare(`INSERT INTO client_documents(client_id,doc_type,file_name,mime_type,file_data) VALUES(?,?,?,?,?)`).bind(clientId,docType,file.name||docType,file.type,data).run();
}
async function intakeStatus(db,token){
  const c=await db.prepare(`SELECT id,full_name,email,phone,address,city,state,zip,id_last4 FROM clients WHERE intake_token=? AND intake_token_expires_at>NOW() LIMIT 1`).bind(token).first();
  if(!c)return null;
  const {results:docs=[]}=await db.prepare(`SELECT doc_type FROM client_documents WHERE client_id=?`).bind(c.id).all();
  return {...c,has_id:docs.some(d=>d.doc_type==="id"),has_bill:docs.some(d=>d.doc_type==="proof_address")};
}
async function submitIntake(db,token,form){
  const c=await db.prepare(`SELECT * FROM clients WHERE intake_token=? AND intake_token_expires_at>NOW() LIMIT 1`).bind(token).first();
  if(!c)return {error:"Este link venció o no es válido.",status:404};
  const last4=String(form.get("id_last4")||"").replace(/\D/g,"");if(last4.length!==4)return {error:"Los últimos 4 del Social deben tener 4 dígitos.",status:400};
  await db.prepare(`UPDATE clients SET full_name=?,email=?,phone=?,address=?,city=?,state=?,zip=?,id_last4=?,intake_completed_at=NOW(),updated_at=NOW() WHERE id=?`)
    .bind(String(form.get("full_name")||"").trim(),String(form.get("email")||"").trim(),String(form.get("phone")||"").trim(),String(form.get("address")||"").trim(),String(form.get("city")||"").trim(),String(form.get("state")||"").trim().toUpperCase(),String(form.get("zip")||"").trim(),last4,c.id).run();
  await saveIntakeImage(db,c.id,"id",form.get("id_file"));await saveIntakeImage(db,c.id,"proof_address",form.get("bill_file"));
  return {ok:true,client_id:Number(c.id)};
}
function roundTemplate(round){return round===1?61:round===2?62:63}
function roundName(round){return round===1?"Round 1 — Initial CRA Review":round===2?"Round 2 — Method / Procedure Follow-up":"Round 3 — Documented Follow-up"}
async function smartStatus(db,clientId){
 const {results:letters=[]}=await db.prepare(`SELECT l.*,t.name AS template_name FROM letters l LEFT JOIN letter_templates t ON t.id=l.template_id WHERE l.client_id=? AND l.notes LIKE 'smart_repair_v15:%' ORDER BY l.round_number,l.recipient_name,l.id`).bind(clientId).all();
 const current=Math.max(1,...letters.map(l=>Number(l.round_number||1)));
 const {results:docs=[]}=await db.prepare(`SELECT id,doc_type FROM client_documents WHERE client_id=?`).bind(clientId).all();
 return {current_round:current,letters,documents:Object.fromEntries(docs.map(d=>[d.doc_type,d.id]))};
}
async function generateSmartRound(db,clientId){
 const {client,items=[]}=await getContext(db,clientId);if(!client)return {error:"Cliente no encontrado.",status:404};
 const st=await smartStatus(db,clientId);const existing=st.letters||[];
 let round=1;
 if(existing.length){
   const cur=Math.max(...existing.map(l=>Number(l.round_number||1)));
   const curLetters=existing.filter(l=>Number(l.round_number||1)===cur);
   if(curLetters.some(l=>!["enviada","entregada","respondida","completada"].includes(l.status)))return {error:`Round ${cur} todavía no está marcado como enviado.`,status:409};
   round=Math.min(3,cur+1);if(cur>=3)return {error:"El caso ya llegó a Round 3. Revisa resultados antes de generar más cartas.",status:409};
 }
 const tpl=await db.prepare(`SELECT * FROM letter_templates WHERE id=? AND is_active=1`).bind(roundTemplate(round)).first();
 if(!tpl)return {error:`Falta la plantilla para Round ${round}.`,status:409};
 const active=items.filter(i=>eligibleCategory(i.category)&&!["removed","deleted","eliminado"].includes(norm(i.removed_status)));
 let generated=0;
 for(const bureau of Object.values(BUREAUS)){
   // Round 1 includes a personal-information review page for each bureau.
   if(round===1){
     const note=`smart_repair_v15:R1:PERSONAL:${bureau.name}`;
     const exists=await db.prepare(`SELECT id FROM letters WHERE client_id=? AND notes=? LIMIT 1`).bind(clientId,note).first();
     if(!exists){
       const city=[client.city,[client.state,client.zip].filter(Boolean).join(" ")].filter(Boolean).join(", ");
       const body=`${new Date().toLocaleDateString("en-US")}\n\n${bureau.name}\n${bureau.address.split("\n").slice(1).join("\n")}\n\nRe: Personal information review\n\nConsumer: ${client.full_name||""}\nCurrent address: ${client.address||""}, ${city}\nLast 4 SSN: ${client.id_last4||""}\n\nPlease review the identifying information associated with my consumer file. Please keep my current identifying information above and investigate any obsolete, duplicate, or inaccurate personal identifiers in my file, including prior addresses, aliases, or telephone variations that are not needed to accurately identify me. Please provide the results of your review in writing.\n\nSincerely,\n${client.full_name||""}`;
       await db.prepare(`INSERT INTO letters(client_id,template_id,title,recipient_name,recipient_address,round_number,body,status,include_id_copy,include_address_proof,include_ssn_copy,notes) VALUES(?,60,?,?,?,?,?,'lista',1,1,0,?)`)
        .bind(clientId,`Personal Information Review — ${bureau.name}`,bureau.name,bureau.address,round,body,note).run();generated++;
     }
   }
   for(const item of active.filter(i=>bureauFor(i.bureaus)?.name===bureau.name)){
     const note=`smart_repair_v15:R${round}:ITEM:${item.id}:${bureau.name}`;
     if(await db.prepare(`SELECT id FROM letters WHERE client_id=? AND notes=? LIMIT 1`).bind(clientId,note).first())continue;
     const body=renderTemplate(tpl.body,mapFor(client,item,bureau));
     await db.prepare(`INSERT INTO letters(client_id,template_id,credit_item_id,title,recipient_name,recipient_address,round_number,body,status,include_id_copy,include_address_proof,include_ssn_copy,notes) VALUES(?,?,?,?,?,?,?,?,'lista',?,?,?,?)`)
      .bind(clientId,tpl.id,item.id,`${roundName(round)} — ${item.creditor_name||categoryLabel(item.category)} — ${bureau.name}`,bureau.name,bureau.address,round,body,tpl.include_id_copy||0,tpl.include_address_proof||0,tpl.include_ssn_copy||0,note).run();generated++;
   }
 }
 return {ok:true,round,generated};
}
async function markBureauSent(db,clientId,bureau,round){
 const r=await db.prepare(`UPDATE letters SET status='enviada',updated_at=NOW() WHERE client_id=? AND round_number=? AND recipient_name LIKE ? AND notes LIKE 'smart_repair_v15:%'`).bind(clientId,round,`%${bureau}%`).run();
 return {ok:true,updated:r?.meta?.changes||0};
}
...
export default{
  async fetch(request,env,ctx){
    ...
    // v15 public client intake
    let intakeHit=u.pathname.match(/^\/api\/intake\/([a-f0-9]{48})$/);
    if(intakeHit&&m==="GET"){try{const x=await intakeStatus(createD1Shim(env),intakeHit[1]);return x?out(x):out({error:"Link vencido o inválido"},404)}catch(e){return out({error:String(e.message||e)},500)}}
    if(intakeHit&&m==="POST"){try{const form=await request.formData();const r=await submitIntake(createD1Shim(env),intakeHit[1],form);return out(r,r.error?(r.status||400):200)}catch(e){return out({error:String(e.message||e)},500)}}
    let v15hit=u.pathname.match(/^\/api\/clients\/(\d+)\/intake-link$/);
    if(v15hit&&m==="POST"){if(!(await auth(request,env,ctx)))return out({error:"No autorizado"},401);const db=createD1Shim(env),token=randomToken();await db.prepare(`UPDATE clients SET intake_token=?,intake_token_expires_at=NOW()+INTERVAL '30 days' WHERE id=?`).bind(token,Number(v15hit[1])).run();return out({url:`${u.origin}/#/intake/${token}`})}
    v15hit=u.pathname.match(/^\/api\/smart-repair\/client\/(\d+)\/status$/);
    if(v15hit&&m==="GET"){if(!(await auth(request,env,ctx)))return out({error:"No autorizado"},401);return out(await smartStatus(createD1Shim(env),Number(v15hit[1])))}
    v15hit=u.pathname.match(/^\/api\/smart-repair\/client\/(\d+)\/generate$/);
    if(v15hit&&m==="POST"){if(!(await auth(request,env,ctx)))return out({error:"No autorizado"},401);const r=await generateSmartRound(createD1Shim(env),Number(v15hit[1]));return out(r,r.error?(r.status||400):200)}
    v15hit=u.pathname.match(/^\/api\/smart-repair\/client\/(\d+)\/bureau\/(Experian|Equifax|TransUnion)\/mark-sent$/);
    if(v15hit&&m==="POST"){if(!(await auth(request,env,ctx)))return out({error:"No autorizado"},401);let body={};try{body=await request.json()}catch{}return out(await markBureauSent(createD1Shim(env),Number(v15hit[1]),v15hit[2],Number(body.round||1)))}
